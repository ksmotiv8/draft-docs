...
On one of our tests, we had both models add a native THROTTLE command (GCRA rate limiter, redis-cell compatible, replication-safe) to Valkey. The token counts tell the story. Kimi wrote 22.7K output tokens, in a single turn, and passed. GLM-5.2-Fast wrote 50.8K output tokens, more than twice as many, across 3 turns, and failed the hidden test suite. You pay for every output token whether it is right or not: an estimated $1.42 for Kimi versus $2.94 for GLM. Kimi was slower on the clock (27 minutes), but the louder model paid double to be wrong. One task, one run apiece, so treat it as an anecdote; the batch numbers are below.
