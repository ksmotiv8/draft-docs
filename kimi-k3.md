...
One test: add a native THROTTLE command to Valkey. Kimi wrote 22.7K output tokens in one turn and passed, for an estimated $1.42. GLM-5.2-Fast wrote 50.8K across three turns and failed, for $2.94. Kimi was slower; the louder model paid double to be wrong. (One run each; the batch numbers are below.)
